...
The \acr{PRC} was established in 1998 by EUROCONTROL’s Permanent Commission.

It provides objective information and independent advice to EUROCONTROL’s governing bodies on European \acr{ATM} performance, based on extensive research, data analysis and consultation with stakeholders. Its purpose is to ensure the effective management of the European air traffic management system through a strong, transparent and independent performance review, as stated in Article 1 of the \acr{PRC} Terms of Reference and Rules of Procedure.

The \acr{PRC} reports to the EUROCONTROL Permanent Commission through the Provisional Council and is supported by the [Performance Review Unit (PRU)](https://ansperformance.eu/about/us/) operating under the EUROCONTROL Agency with the appropriate level of independence.

For more information about the \acr{PRC}, see [here](https://ansperformance.eu/about/prc/).

## Open Performance Data Initiative

In 2022, the \acr{PRC} launched the \acr{OPDI} with a view to establish a data platform, to support higher levels of transparency and reproducibility of performance related monitoring and associated analyses. This initiative addresses the requirement to establish a level playing field for all stakeholders, ranging from the interested public, operational and strategic planners, to political decision-makers. Pursuing an open data-based approach will allow all interested parties to tap into a harmonised data environment to build their assessments, validate results published by other parties, and reproduce findings in support of the political and operational discussion about observed performance.
...
For more information about the \acr{OPDI}, see [here](https://www.eurocontrol.int/publication/performance-insight-5-open-performance-data).

## PRC Data Challenges

The PRC Data Challenges are an annual competition organised by the \acr{PRC} that invites data scientists, researchers, and aviation enthusiasts to solve real-world aviation performance questions using open data. Each edition defines a specific prediction or estimation task, provides curated datasets, and evaluates submissions on a public leaderboard.

By engaging a diverse community of participants, the PRC Data Challenges advance the state of the art in aviation analytics while demonstrating the practical value of open performance data. Past editions have tackled takeoff weight prediction (2024) and fuel flow estimation (2025).

See the [Data Challenges](data-challenges.qmd) page for details on each edition, or visit the [PRC Data Challenge programme page](https://ansperformance.eu/study/data-challenge/).

## OPDI Sensors

The OPDI Sensors is an initiative to extend the [OpenSky Network](https://opensky-network.org/) crowd-sourced receiver network by deploying ground-based ADS-B sensors at strategic airport locations. The aim is to improve ground-based and low-altitude ADS-B coverage around airports, especially where existing coverage is limited. The data collected feeds directly into the OPDI, improving the quality and completeness of open flight performance data.

The programme currently includes 12 sensors feeding into the OpenSky Network across 7 airports in 4 countries, with plans to expand to additional locations. Airport operators and aviation stakeholders interested in hosting a sensor are invited to [get in touch](mailto:pru-support@eurocontrol.int).

See the [OPDI Sensors](sensors.qmd) page for the full deployment overview and information on hosting a sensor.

## Benefits of Open Access and Open Data for Performance Monitoring

To understand the depth of the proposed \acr{OPDI} it is vital to define the key terminology upfront.
...
* **Open Data**: The term Open Data refers to data—typically managed and available online—which is free of cost and accessible. Open data can be freely used, reused, and distributed provided that the data source is attributed.
* **Open-Source Software**: Open-Source Software is software whose source code can be openly accessed, inspected, modified, and enhanced by anyone.

The focus of the \acr{OPDI} is on open data, in particular open access and open use of data for non-commercial purposes. It is understood that potential developments of novel algorithms and techniques can produce tools that are made available to the community as open-source software.
...
A key tenet of the performance-based approach is independent and impartial monitoring to provide the evidence for achieved performance levels and to identify and share best practice for further performance enhancements in \acr{ATM}.
...
Open data from both public and private sources are adding a new dimension to the data revolution. It also is a key ingredient to leverage the promises of big data, or supporting data-analytical advances in artificial intelligence/machine-learning.
...
### Effectiveness
At the heart of the performance-based approach is the potential to achieve higher levels of performance and associated decision-making through data-driven decision-making. This may affect day-to-day decision-making, but also long-term decisions such as the need for adopting new operational concepts or deploying operational capabilities.
...
For questions about any OPDI activity — data, challenges, or sensors — contact [pru-support@eurocontrol.int](mailto:pru-support@eurocontrol.int).
