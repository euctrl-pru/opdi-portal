...
![](../assets/img/opensky-network-logo_compact.jpg)
...
* The `version` indicates the algorithm version used to detect the flight event. For more info, see [Methodology](methodology.qmd).
...
![(Simplified) Flight phases and events. For a more complete/complex representation, see @fig-flight-phases_complex in the collapsable block](../assets/img/aircraft_state_diagram.png){#fig-flight-phases width="100%"}
...
![Flight phases and events, a more complete/complex diagram.](../assets/img/aircraft_state_diagram_complex.png){#fig-flight-phases_complex width="100%"}
...
(@) Off-block (**T04**) — published, as `off-block`
(@) End of push back (**T05**)
(@) Enter runway for take-off (**T06**) — published, as `line-up`
(@) Start of take-off roll (**T07**) — published, as `take-off-roll`
(@) Lift-off (**T08**), a.k.a. take-off — published, as `airborne`
(@) 40-nautical-miles intersection (sort of **T11**)
(@) Top-of-climb (**T12**)
(@) Top-of-descent (**T13**)
(@) 40-nautical-miles intersection (sort of **T14**)
(@) Over runway threshold (**T16**) — published, as `landing`
(@) Touch-down (**T17**) — published, as `touchdown`
(@) Runway vacated (**T19**) — published, as `runway-vacated`
(@) Enter parking spot (**T20**)
(@) On-block (**T21**) — published, as `on-block`

The items marked "published" above ship under `events_v0.2.0`, the A-CDM
runway family described in [Flight events from ADS-B — version
4](../papers/flight-events-v4/index.html). Two more members of that same family
have no ICAO T-number at all, because they answer a question the T00–T25
ladder never asks — a runway *crossing* rather than a runway *operation* —
and one covers an outcome the ladder has no room for:

(@) Go-around — published, as `go-around`
(@) Runway crossing, entry — published, as `runway-crossing-entry`
(@) Runway crossing, vacated — published, as `runway-crossing-vacated`
...
![Milestones for gate to gate.](../assets/img/milestones_3d.png)
Possible monitoring usages are:

* Airport performance (throughput, RWY utilization, runway occupancy)
* Fuel efficiency & CO2/NOx emissions
* Airline profiling
* Safety: separation and spacing
...
![LTO phases ([@doi/10.2822/385503] Figure 2.5).](../assets/img/lto-cycle_eea.png)
