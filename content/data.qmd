...
**Coverage:** `r labs$month_range(cfg$coverage$start, cfg$coverage$end)` | **Version:** `r cfg$version`
:::
...
The current release covers `r labs$month_range(cfg$coverage$start, cfg$coverage$end)`. Further enhancements are planned according to our [Roadmap](roadmap.html).

Download the data here: 

| Table Reference | Description   | Data Source(s)     | Version | Download Link / Portal                          |
|-----------------|---------------|--------------------|---------|-----------------------------------------|
| `FLIGHT`        | Flight list   | [OSN]/[PRC]        | `r cfg$version` | [Data portal][FListpq]  |
| `EVENT`         | Flight events | [OSN]/[PRC]        | `r cfg$version` | [Data portal][Eventspq] |
| `MEASUREMENT`   | Measurements  | [OSN]/[PRC]        | `r cfg$version` | [Data portal][Measpq]    |
...
![Bounding box for OPDI state vector coverage, SW=[-25.86653, 26.74617] NE=[49.65699, 70.25976]](../assets/img/bbox-opdi.png "Title: a map of the bounding box for OPDI state vector coverage"){width="80%" fig-alt="Bounding box for OPDI state vector coverage."}
