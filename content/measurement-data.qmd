...
cfg <- portal_cfg
labs <- portal_labels
start_date <- cfg$coverage$start
end_date <- cfg$coverage$end
version <- cfg$version
```

::: {.callout-note appearance="simple"}
**Coverage:** `r labs$date_range(start_date, end_date)` | **Version:** `r version`
:::

Below you will find links to download measurements `r version` data files from `r format(as.Date(start_date), "%B %d, %Y")` to `r format(as.Date(end_date), "%B %d, %Y")`. Each file is in Parquet format, providing a highly efficient way to store and manage large datasets. The datasets are separated into 10-day intervals. Please note that each dataset covers data up to, but not including, the last day listed in its name.

## Automated Download

If you want to automate your downloads, you can generate the link for downloading using the following format where `YYYYMMDD` is the start date of the 10-day period. Note that the first date to start generating intervals for is `r format(as.Date(start_date), "%B %d, %Y")`. 
...
if __name__ == "__main__":
    datasets = {
        ...
        #"flight_events": ("`r format(cfg$coverage$start, "%Y%m%d")`", "`r format(cfg$coverage$end, "%Y%m%d")`")#,
        "measurements": ("`r format(start_date, "%Y%m%d")`", "`r format(end_date, "%Y%m%d")`")
    }
...
datasets <- list(
  ...
  #"flight_events" = c("`r format(cfg$coverage$start, "%Y%m%d")`", "`r format(cfg$coverage$end, "%Y%m%d")`")#,
  "measurements" = c("`r format(start_date, "%Y%m%d")`", "`r format(end_date, "%Y%m%d")`")
)
...
generate_markdown_entries <- function(start_date, end_date, interval_days = 10) {
  base_url <- "https://www.eurocontrol.int/performance/data/download/OPDI/v002/measurements/"
  markdown_entries <- c()
  ...
  while (current_date < end_date) {
    ...
    entry <- paste0(
      ...
      "[Download measurements_", format(current_date, "%Y%m%d"), "_", format(next_date, "%Y%m%d"), ".parquet](", 
      base_url, "measurements_", format(current_date, "%Y%m%d"), "_", format(next_date, "%Y%m%d"), ".parquet)"
    )
...
We hope you find these datasets useful for your analysis and research. If you have any questions or require further assistance, please contact us at [pru-support@eurocontrol.int](mailto:pru-support@eurocontrol.int).
